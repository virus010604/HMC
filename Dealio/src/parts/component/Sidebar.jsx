...
export default function Sidebar({ isVisible, toggleSidebar, refreshSidebar }) {
  ...
  return (
    <div className="relative space-y-5">
      <div
        className={`overflow-auto pt-24 pb-10 px-5 shadow-lg h-full lg:w-[400px] w-[260px] bg-white fixed right-0 bottom-24 top-0 z-20 transition-transform duration-300 ${
        ...
        {data.map((item, index) => (
          <div key={index} className="">
            <div className="grid grid-flow-row-1 gap-2 p-4 border my-2">
              ...
              <h2 className="font-semibold text-[#572dff] text-sm">
                ${item.price}
              </h2>
            </div>
          </div>
